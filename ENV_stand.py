...
class SA_MS_ENV():
    def __init__(self, space, stands, action_bound, target_magnet_points):
        self.space = space
        self.agents = stands
        self.action_bound = action_bound
        self.action_space = Action_Space(action_bound)
        ...
        # D = np.sum(abs(self.stand_magnet_state[:30] - self.target_magnet_points[:30]))
        self.D = D
        self.space.step(.0001)
    ...
    def reset(self):
        ...
        #         action = np.random.choice(6)
        #         agent.body_move(action, (.001, .0001))
        self.agents[0].continuous_body_move([-1,  0, 0])
        self.agents[1].continuous_body_move([-1, -1, 0])
        self.agents[2].continuous_body_move([ 0, -1, 0])
        self.agents[3].continuous_body_move([ 1, -1, 0])
        self.agents[4].continuous_body_move([ 1,  0, 0])
        # self.agents[5].continuous_body_move([ 5,  5, 0])
    ...
    def step(self, action):
        ...
        for i in range(len(self.agents) - 1):
            self.agents[i].continuous_body_move(action[i])
        ...
        # self.D = np.sum(abs(self.stand_magnet_state[:30] - self.target_magnet_points[:30]))
    ...
    def terminal(self):
        terminal = False
        abs_diff = abs(self.stand_magnet_state[:30] - self.target_magnet_points[:30])
        x_abs_diff = abs_diff[:,0]
        y_abs_diff = abs_diff[:,1]
        if np.all(x_abs_diff < 0.014) and np.all(y_abs_diff < 0.013):
            terminal = True
        return terminal
